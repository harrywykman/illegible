...
import loader_functions.initialise_new_game as initialise
...
def play_game(player, entities, game_map, message_log, game_state, con, panel, constants):
    ...
    while not libtcod.console_is_window_closed():
        ...
        if fov_recompute:
            fov_functions.recompute_fov(fov_map, player.x, player.y, constants['fov_radius'], constants['fov_light_walls'],
                          constants['fov_algorithm'])

        render_functions.render_all(con, panel, entities, player, game_map, fov_map, fov_recompute, message_log,
...
def main():
    constants = initialise.get_constants()
    ...
    while not libtcod.console_is_window_closed():
        ...
        if show_main_menu:
            menus.main_menu(con, main_menu_background_image, constants['screen_width'],
                      constants['screen_height'])

            if show_load_error_message:
                menus.message_box(con, 'No save game to load', 50, constants['screen_width'], constants['screen_height'])
            ...
            if show_load_error_message and (new_game or load_saved_game or exit_game):
                ...
                player, entities, game_map, message_log, game_state = initialise.get_game_variables(constants)
                game_state = game_states.GameStates.PLAYERS_TURN
